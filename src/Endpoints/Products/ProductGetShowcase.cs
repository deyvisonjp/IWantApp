...
public class ProductGetShowcase
{
    ...
    {
        //Padrão caso o usuário não selecione numero de páginas e linhas
        if (row > 10)
            return Results.Problem(title: "Row with max 10", statusCode: 400);

        var queryBase = context.Products.AsNoTracking().Include(p => p.Category) //AsNoTracking -> Mais performatico já que esse controller é somente consulta
